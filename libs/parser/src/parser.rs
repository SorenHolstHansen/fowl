...
struct Parser<'source> {
    tokens: Peekable<IntoIter<(Token<'source>, Range<usize>)>>,
...
impl<'source> Parser<'source> {
    ...
    fn expect_token(&mut self, token: Token) -> Result<(Token<'source>, Span), Diagnostic> {
        match self.tokens.next() {
            Some((t, span)) if token == t => Ok((t, span.into())),
    ...
    fn expect_one_of_token(
        ...
        match self.tokens.next() {
            Some((t, span)) if tokens.contains(&t) => Ok((t, span.into())),
            Some((t, span)) => Err(Diagnostic::error(
                span.into(),
                format!(
    ...
    fn parse_type(&mut self) -> Result<Type<'source>, Diagnostic> {
        match self.tokens.next() {
            ...
            Some((_, span)) => Err(Diagnostic::error(span.into(), "expected type")),
    ...
    fn parse_ident(&mut self) -> Result<Ident<'source>, Diagnostic> {
        match self.tokens.next() {
            Some((Token::Ident(name), span)) => Ok(Ident {
                inner: name,
                span: span.into(),
            }),
            Some((_, span)) => Err(Diagnostic::error(span.into(), "expected identifier")),
            None => {
    ...
    fn parse_declaration(&mut self) -> Result<Declaration<'source>, Diagnostic> {
        ...
        match token {
            Token::Struct => {
                ...
                loop {
                    if matches!(self.tokens.peek(), Some((Token::RBrace, _))) {
                        // struct end
                        self.tokens.next();
                        break;
                    }
                    ...
                    if let Some((Token::RBrace, span)) = self.tokens.peek() {
                        struct_span = struct_span.merge_range(span);
                        self.tokens.next();
                        break;
                    }
                }
            ...
            Token::Enum => {
                ...
                loop {
                    ...
                    if let Some((Token::RBrace, span)) = self.tokens.peek() {
                        enum_span = enum_span.merge_range(span);
                        self.tokens.next();
                        break;
                    }
                }
    ...
    fn parse_function(&mut self) -> Result<Function<'source>, Diagnostic> {
        ...
        let mut function_span = Span::from(span);
    ...
    fn parse_function_parameters(&mut self) -> Result<Vec<Param<'source>>, Diagnostic> {
        ...
        if matches!(self.tokens.peek(), Some((Token::RParen, _))) {
    ...
    fn parse_statements(&mut self) -> Result<Vec<Statement<'source>>, Diagnostic> {
        ...
        loop {
            if matches!(self.tokens.peek(), None | Some((Token::RBrace, _))) {
                break;
            };
    ...
    pub fn parse_expression(&mut self, min_bp: u8) -> Result<Expr<'source>, Diagnostic> {
        ...
        let expr_span = Span::from(span.clone());
        ...
        loop {
            let Some((op_token, span)) = self.tokens.peek() else {
                break;
            };
